...
public final class MultipartFileUtils {
	...
	public static List<File> saveFiles(HttpServletRequest request,
			String dirPath) throws IllegalStateException, IOException {
		File dir = new File(dirPath);
		return saveFiles(request, dir);
	}

	public static List<File> saveFilesWithTimestamp(HttpServletRequest request,
			String dirPath) throws IllegalStateException, IOException {
		File dir = new File(dirPath);
		return saveFilesWithTimestamp(request, dir);
	}

	public static List<File> saveFiles(HttpServletRequest request, File dir)
			throws IllegalStateException, IOException {
		return saveFilesWithTimestamp(request, dir);
	}

	public static List<File> saveFilesWithTimestamp(HttpServletRequest request,
			File dir) throws IllegalStateException, IOException {
	...
	public static List<File> saveFiles(MultipartHttpServletRequest request,
			String dirPath) throws IllegalStateException, IOException {
		File dir = new File(dirPath);
	...
	public static List<File> saveFilesWithTimestamp(
			MultipartHttpServletRequest request, File dir)
			throws IllegalStateException, IOException {
		return saveFilesWithTimestamp(request, dir, new DateTime());
	}

	public static List<File> saveFilesWithTimestamp(
			MultipartHttpServletRequest request, File dir, DateTime dateTime)
			throws IllegalStateException, IOException {
		...
		if (!dir.exists()) {
			...
			logger.debug("create directory '" + dir.getAbsolutePath()
					+ "' ok? " + result);
			if (!result) {
		...
		for (String fileName : fileNames) {
			...
			for (MultipartFile file : files) {
				...
				// 上传
				String savedPath = Joiner.on(
						System.getProperty("file.separator")).join(
						dir.getAbsolutePath(), originalFileName);
				logger.debug("save to " + savedPath);
				File saved = new File(savedPath);
				if (dateTime != null) {
					saved = addTimestamp(saved, DEFAULT_FLAG,
							DEFAULT_PREFIX_BEFORE_TIMESTAMP, dateTime);
				}
	...
	static File addTimestamp(File file, String flag,
			String prefixBeforeTimestamp, DateTime dateTime) {
	...
	static void checkUploadedFileNames(Collection<String> fileNames) {
		boolean result = areValidUploadedFileNames(fileNames);
		logger.debug("uploaded files' names are all valid? " + result);
		if (!result) {
			throw new RuntimeException(
					"invalid characters found in the uploaded file names");
		}
	}

	static boolean areValidUploadedFileNames(Collection<String> fileNames) {
	...
	static boolean isValidUploadedFileName(String fileName) {
