...
@RestController
public class AssetsI18nController {

	@Autowired
	private AppConfig appConfig;

	@RequestMapping(value = RouteDefine.ASSETS_VERSION + "/i18n/**", method = RequestMethod.GET)
	public void get(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		String uri = request.getRequestURI();
		String path = FilePathTool.join(appConfig.getDataDir(), uri);
		File file = new File(path);
		HttpResponseTool.writeFile("text/palin", response, file);
	}
